...
//	ftl_log("check111\n");	
	...
	ftl_log("hp_cnt: %ld   zcl_cnt: %ld\n", hp_cnt, zcl_cnt);
	ftl_log("check_dpg_maptbl_ent_cnt: %ld  %ld\n", idx, check_cnt); 

	if (hp_cnt + zcl_cnt != check_cnt)
		ftl_log("check count err..\n");
...
{ 
	...
	//	while (bp->status->tt_reqs > 64 && bp->dpg_flush_list->reqs <= 64) { 
		...
		while (bp->status->tt_reqs > 522428 && bp->cond_chip->idle == 1) {
			//ftl_log("start\n");
...
{
#ifdef FG_DEBUG
	ftl_log("ssd_buff_flush_with_fifo...\n"); 
#endif
	struct ssd_buff *bp = &ssd->buff; 
	//int64_t curlat = 0, maxlat = 0; 
	//uint64_t accumulat = 0;
	...
	if (bp->dpg_to_flush_list->head == NULL) 
...
{ 
	...
	for (lpn = start_lpn; lpn <= end_lpn; lpn++) { 
...
#endif
		//ftl_log("tt_reqs: %d  bp_reqs: %ld\n", bp->status->tt_reqs, bp->bp_reqs);
		...
		} else if (bp->status->tt_reqs > BUFF_SIZE) { 
			//ftl_log("2222\n");
			while (1) { 
				//ftl_log("2222\n");
				usleep(10000);
