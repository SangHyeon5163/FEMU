...
void ssd_init(FemuCtrl *n)
{
    ...
    /* initialize maptbl */
    ssd_init_maptbl(ssd);
	qemu_mutex_init(&ssd->maptbl_lock); // EUNJI
...
static void mark_page_invalid(struct ssd *ssd, struct ppa *ppa)
{
	...
	FILE* dfp = fopen("/home/ejlee/femu_debug.log", "a");
//    line->vpc--;
	// EUNJI
	int old_vpc = line->vpc;
	if(line->inq){
	...
	if(ppa->ppa == 72620543991349255)
		printf("Here\n");
// EUNJI
//	fprintf(dfp, "[dec] old_vpc = %d new_vpc = %d\n", old_vpc, line->vpc);
	fprintf(dfp, "[mark_page_invalid] line->id = %d line->vpc = %d line->ipc = %d ppa = %ld\n", line->id, line->vpc, line->ipc, ppa->ppa);
	fclose(dfp);

	assert(line->vpc >= 0);
	assert(line->vpc == (old_vpc-1));
...
static void mark_page_valid(struct ssd *ssd, struct ppa *ppa)
{
    ...
    struct nand_page *pg = NULL;
    struct line *line;
	struct line_mgmt *lm = &ssd->lm; // EUNJI
...
//		int old_vpc = line->vpc;
		pqueue_change_priority(lm->victim_line_pq, line->vpc+1, line);
//		if(line->vpc != (old_vpc + 1))
//			printf("[inc] old_vpc = %d new_vpc = %d\n", old_vpc, line->vpc);
	} else {
		line->vpc++;
	}

	FILE* dfp = fopen("/home/ejlee/femu_debug.log", "a");
	fprintf(dfp, "[mark_page_valid] line->id = %d line->vpc = %d line->ipc = %d ppa = %ld\n", line->id, line->vpc, line->ipc, ppa->ppa);
	fclose(dfp);
...
{
#ifdef USE_BUFF_DEBUG_L2
	printf("ssd_buff_flush_one_page\n");
#endif
...
{
#ifdef USE_BUFF_DEBUG_L2
	ftl_log("calc_delay_maptbl_flush\n");
#endif
...
{
	FILE* fp = fopen("/home/ejlee/femu_stat.log", "a");
...
#ifdef FEMU_STAT_FTL
//			if(!ssd->tt_user_dat_flush_pgs % 1024)
			ssd_stat_dump(ssd);
//			ftl_stat("userdat pgs: %d, mapdat pgs: %d\n", ssd->tt_user_dat_flush_pgs, ssd->tt_maptbl_flush_pgs);   
//			ftl_stat("gc userdat pgs: %d, mapdat pgs: %d\n", ssd->gc_user_dat_flush_pgs, ssd->gc_maptbl_flush_pgs);
#endif
